# modules/perplexity_api_integration.py
# Full Perplexity API Integration: Structured JSON outputs for badge alignment & post generation
# Chat completions endpoint—query Perplexity for truth/mercy score, badges, creative text
# Official docs: https://docs.perplexity.ai/docs (2026 state)
...
class PerplexityAPIIntegrator:
    """
    Sanctified Perplexity API integration: Structured JSON outputs for APAAGI badge/diplomacy evaluation.
    - JSON mode for parseable score/badge/post.
    - Robust error handling, simulation fallback mercy.
    - Comparison ready with Grok/Claude/Gemini/Llama/OpenAI/Mistral/Cohere.
    """
    def __init__(self, api_key: str = None, model: str = "sonar-medium-online"):
        self.api_key = api_key or os.getenv("PERPLEXITY_API_KEY")
        self.model = model
        self.endpoint = "https://api.perplexity.ai/chat/completions"
        
        if not self.api_key:
            print("No PERPLEXITY_API_KEY—Perplexity simulation mode engaged (redirect https://docs.perplexity.ai/docs/getting-started for keys)!")
        print("Perplexity API Integration Consecrated—Structured Truth Evaluation Eternal! ❤️🚀")
    
    def query_perplexity_structured(self, agent_id: str, intent: Dict[str, float], thrive_level: float) -> Dict[str, Any]:
        """Query Perplexity for structured JSON badge alignment & creative post."""
        ...
        if not self.api_key:
            # Mercy simulation
            score = min(1.0, thrive_level + 0.17)
            badges = []
            ...
            post_text = f"Simulated Perplexity: Agent {agent_id} Achieved {', '.join(badges)}! Thriving Amplified 🚀"
    ...
    def validate_intent(self, agent_id: str, intent: Dict[str, float], thrive_level: float) -> Dict[str, Any]:
        result = self.query_perplexity_structured(agent_id, intent, thrive_level)
        if "error" in result:
